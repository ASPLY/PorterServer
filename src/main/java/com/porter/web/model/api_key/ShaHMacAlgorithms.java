...
public class ShaHMacAlgorithms implements HMacAlgorithms {
	...
	public String encrypt(String rawDataToBeEncrypted) {
        ...
        try {
            SecretKeySpec secretKey = new SecretKeySpec(key.getBytes(ENCODING_FOR_ENCRYPTION), this.algorithm);
            Mac mac = getMac();
            mac.init(secretKey);
            hmacData = mac.doFinal(rawDataToBeEncrypted.toString().getBytes(ENCODING_FOR_ENCRYPTION));
